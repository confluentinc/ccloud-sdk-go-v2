// Copyright 2021 Confluent Inc. All Rights Reserved.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

/*
SQL API v1

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 0.0.1
Contact: flink-control-plane@confluent.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package v1

import (
	"bytes"
	"encoding/json"
)

import (
	"reflect"
)

// SqlV1ColumnCommon struct for SqlV1ColumnCommon
type SqlV1ColumnCommon struct {
	// The name of the column.
	Name string   `json:"name,omitempty"`
	Type DataType `json:"type,omitempty"`
	// A comment or description for the column.
	Comment *string `json:"comment,omitempty"`
}

// NewSqlV1ColumnCommon instantiates a new SqlV1ColumnCommon object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSqlV1ColumnCommon(name string, type_ DataType) *SqlV1ColumnCommon {
	this := SqlV1ColumnCommon{}
	this.Name = name
	this.Type = type_
	return &this
}

// NewSqlV1ColumnCommonWithDefaults instantiates a new SqlV1ColumnCommon object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSqlV1ColumnCommonWithDefaults() *SqlV1ColumnCommon {
	this := SqlV1ColumnCommon{}
	return &this
}

// GetName returns the Name field value
func (o *SqlV1ColumnCommon) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *SqlV1ColumnCommon) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *SqlV1ColumnCommon) SetName(v string) {
	o.Name = v
}

// GetType returns the Type field value
func (o *SqlV1ColumnCommon) GetType() DataType {
	if o == nil {
		var ret DataType
		return ret
	}

	return o.Type
}

// GetTypeOk returns a tuple with the Type field value
// and a boolean to check if the value has been set.
func (o *SqlV1ColumnCommon) GetTypeOk() (*DataType, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Type, true
}

// SetType sets field value
func (o *SqlV1ColumnCommon) SetType(v DataType) {
	o.Type = v
}

// GetComment returns the Comment field value if set, zero value otherwise.
func (o *SqlV1ColumnCommon) GetComment() string {
	if o == nil || o.Comment == nil {
		var ret string
		return ret
	}
	return *o.Comment
}

// GetCommentOk returns a tuple with the Comment field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SqlV1ColumnCommon) GetCommentOk() (*string, bool) {
	if o == nil || o.Comment == nil {
		return nil, false
	}
	return o.Comment, true
}

// HasComment returns a boolean if a field has been set.
func (o *SqlV1ColumnCommon) HasComment() bool {
	if o != nil && o.Comment != nil {
		return true
	}

	return false
}

// SetComment gets a reference to the given string and assigns it to the Comment field.
func (o *SqlV1ColumnCommon) SetComment(v string) {
	o.Comment = &v
}

// Redact resets all sensitive fields to their zero value.
func (o *SqlV1ColumnCommon) Redact() {
	o.recurseRedact(&o.Name)
	o.recurseRedact(&o.Type)
	o.recurseRedact(o.Comment)
}

func (o *SqlV1ColumnCommon) recurseRedact(v interface{}) {
	type redactor interface {
		Redact()
	}
	if r, ok := v.(redactor); ok {
		r.Redact()
	} else {
		val := reflect.ValueOf(v)
		if val.Kind() == reflect.Ptr {
			val = val.Elem()
		}
		switch val.Kind() {
		case reflect.Slice, reflect.Array:
			for i := 0; i < val.Len(); i++ {
				// support data types declared without pointers
				o.recurseRedact(val.Index(i).Interface())
				// ... and data types that were declared without but need pointers (for Redact)
				if val.Index(i).CanAddr() {
					o.recurseRedact(val.Index(i).Addr().Interface())
				}
			}
		}
	}
}

func (o SqlV1ColumnCommon) zeroField(v interface{}) {
	p := reflect.ValueOf(v).Elem()
	p.Set(reflect.Zero(p.Type()))
}

func (o SqlV1ColumnCommon) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["name"] = o.Name
	}
	if true {
		toSerialize["type"] = o.Type
	}
	if o.Comment != nil {
		toSerialize["comment"] = o.Comment
	}
	buffer := &bytes.Buffer{}
	encoder := json.NewEncoder(buffer)
	encoder.SetEscapeHTML(false)
	err := encoder.Encode(toSerialize)
	return buffer.Bytes(), err
}

type NullableSqlV1ColumnCommon struct {
	value *SqlV1ColumnCommon
	isSet bool
}

func (v NullableSqlV1ColumnCommon) Get() *SqlV1ColumnCommon {
	return v.value
}

func (v *NullableSqlV1ColumnCommon) Set(val *SqlV1ColumnCommon) {
	v.value = val
	v.isSet = true
}

func (v NullableSqlV1ColumnCommon) IsSet() bool {
	return v.isSet
}

func (v *NullableSqlV1ColumnCommon) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSqlV1ColumnCommon(val *SqlV1ColumnCommon) *NullableSqlV1ColumnCommon {
	return &NullableSqlV1ColumnCommon{value: val, isSet: true}
}

func (v NullableSqlV1ColumnCommon) MarshalJSON() ([]byte, error) {
	buffer := &bytes.Buffer{}
	encoder := json.NewEncoder(buffer)
	encoder.SetEscapeHTML(false)
	err := encoder.Encode(v.value)
	return buffer.Bytes(), err
}

func (v *NullableSqlV1ColumnCommon) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
