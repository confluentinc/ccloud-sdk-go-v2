...
> NetworkingV1PeeringList ListNetworkingV1Peerings(ctx).Environment(environment).DisplayName(displayName).Network(network).Phase(phase).PageSize(pageSize).PageToken(pageToken).Execute()
...
func main() {
    ...
    configuration := openapiclient.NewConfiguration()
    api_client := openapiclient.NewAPIClient(configuration)
    resp, r, err := api_client.PeeringsNetworkingV1Api.ListNetworkingV1Peerings(context.Background()).Environment(environment).DisplayName(displayName).Network(network).Phase(phase).PageSize(pageSize).PageToken(pageToken).Execute()
    if err != nil {
