...
**Target** | Pointer to **string** | The name of the endpoint that is currently active; always one of &#x60;endpoints[].name&#x60;. Output-only: the Switchover service derives it from the pair&#39;s active member at create time and moves it on every failover. A value supplied on create is ignored.  | [optional] [readonly] 
