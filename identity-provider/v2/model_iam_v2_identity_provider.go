...
type IamV2IdentityProvider struct {
	...
	// The JWKS provided by the Provider. We only express the `kid` and `algorithm` for each key set
	Keys *[]IamV2Jwks `json:"keys,omitempty"`
	// Indicates whether the current provider is disabled. All corresponding `IdentityPool` objects would also be disabled 
