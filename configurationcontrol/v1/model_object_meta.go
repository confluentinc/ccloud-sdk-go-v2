...
type ObjectMeta struct {
	...
	// Client-supplied stable identifier for the policy. Used as the URL identifier and must be unique within the caller's organization. Immutable.
	Name string `json:"name,omitempty"`
	// System-generated unique identifier for this policy. Distinct from `name`: while `name` is the client-supplied URL identifier, `uid` remains unique across time even if a policy with the same `name` is deleted and recreated.
	Uid *string `json:"uid,omitempty"`
	// Opaque value identifying the current version of the policy.
