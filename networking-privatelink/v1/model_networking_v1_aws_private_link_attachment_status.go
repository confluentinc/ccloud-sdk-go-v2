// Copyright 2021 Confluent Inc. All Rights Reserved.
//
// Licensed under the Apache License, Version 2.0 (the "License");
// you may not use this file except in compliance with the License.
// You may obtain a copy of the License at
//
//     http://www.apache.org/licenses/LICENSE-2.0
//
// Unless required by applicable law or agreed to in writing, software
// distributed under the License is distributed on an "AS IS" BASIS,
// WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
// See the License for the specific language governing permissions and
// limitations under the License.

/*
Network API

Network API

API version: 0.0.1-alpha1
Contact: cire-traffic@confluent.io
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package v1

import (
	"encoding/json"
)

import (
	"reflect"
)

// NetworkingV1AwsPrivateLinkAttachmentStatus AWS PrivateLink attachment represents reserved capacity in an AWS VPC Endpoint Service that can be used to establish PrivateLink connections.
type NetworkingV1AwsPrivateLinkAttachmentStatus struct {
	// PrivateLinkAttachmentStatus kind.
	Kind string `json:"kind"`
	// AWS VPC Endpoint Service that can be used to establish connections for all zones.
	VpcEndpointService NetworkingV1AwsVpcEndpointService `json:"vpc_endpoint_service"`
}

// NewNetworkingV1AwsPrivateLinkAttachmentStatus instantiates a new NetworkingV1AwsPrivateLinkAttachmentStatus object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewNetworkingV1AwsPrivateLinkAttachmentStatus(kind string, vpcEndpointService NetworkingV1AwsVpcEndpointService) *NetworkingV1AwsPrivateLinkAttachmentStatus {
	this := NetworkingV1AwsPrivateLinkAttachmentStatus{}
	this.Kind = kind
	this.VpcEndpointService = vpcEndpointService
	return &this
}

// NewNetworkingV1AwsPrivateLinkAttachmentStatusWithDefaults instantiates a new NetworkingV1AwsPrivateLinkAttachmentStatus object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewNetworkingV1AwsPrivateLinkAttachmentStatusWithDefaults() *NetworkingV1AwsPrivateLinkAttachmentStatus {
	this := NetworkingV1AwsPrivateLinkAttachmentStatus{}
	return &this
}

// GetKind returns the Kind field value
func (o *NetworkingV1AwsPrivateLinkAttachmentStatus) GetKind() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Kind
}

// GetKindOk returns a tuple with the Kind field value
// and a boolean to check if the value has been set.
func (o *NetworkingV1AwsPrivateLinkAttachmentStatus) GetKindOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Kind, true
}

// SetKind sets field value
func (o *NetworkingV1AwsPrivateLinkAttachmentStatus) SetKind(v string) {
	o.Kind = v
}

// GetVpcEndpointService returns the VpcEndpointService field value
func (o *NetworkingV1AwsPrivateLinkAttachmentStatus) GetVpcEndpointService() NetworkingV1AwsVpcEndpointService {
	if o == nil {
		var ret NetworkingV1AwsVpcEndpointService
		return ret
	}

	return o.VpcEndpointService
}

// GetVpcEndpointServiceOk returns a tuple with the VpcEndpointService field value
// and a boolean to check if the value has been set.
func (o *NetworkingV1AwsPrivateLinkAttachmentStatus) GetVpcEndpointServiceOk() (*NetworkingV1AwsVpcEndpointService, bool) {
	if o == nil {
		return nil, false
	}
	return &o.VpcEndpointService, true
}

// SetVpcEndpointService sets field value
func (o *NetworkingV1AwsPrivateLinkAttachmentStatus) SetVpcEndpointService(v NetworkingV1AwsVpcEndpointService) {
	o.VpcEndpointService = v
}

// Redact resets all sensitive fields to their zero value.
func (o *NetworkingV1AwsPrivateLinkAttachmentStatus) Redact() {
	o.recurseRedact(&o.Kind)
	o.recurseRedact(&o.VpcEndpointService)
}

func (o *NetworkingV1AwsPrivateLinkAttachmentStatus) recurseRedact(v interface{}) {
	type redactor interface {
		Redact()
	}
	if r, ok := v.(redactor); ok {
		r.Redact()
	} else {
		val := reflect.ValueOf(v)
		if val.Kind() == reflect.Ptr {
			val = val.Elem()
		}
		switch val.Kind() {
		case reflect.Slice, reflect.Array:
			for i := 0; i < val.Len(); i++ {
				// support data types declared without pointers
				o.recurseRedact(val.Index(i).Interface())
				// ... and data types that were declared without but need pointers (for Redact)
				if val.Index(i).CanAddr() {
					o.recurseRedact(val.Index(i).Addr().Interface())
				}
			}
		}
	}
}

func (o NetworkingV1AwsPrivateLinkAttachmentStatus) zeroField(v interface{}) {
	p := reflect.ValueOf(v).Elem()
	p.Set(reflect.Zero(p.Type()))
}

func (o NetworkingV1AwsPrivateLinkAttachmentStatus) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if true {
		toSerialize["kind"] = o.Kind
	}
	if true {
		toSerialize["vpc_endpoint_service"] = o.VpcEndpointService
	}
	return json.Marshal(toSerialize)
}

type NullableNetworkingV1AwsPrivateLinkAttachmentStatus struct {
	value *NetworkingV1AwsPrivateLinkAttachmentStatus
	isSet bool
}

func (v NullableNetworkingV1AwsPrivateLinkAttachmentStatus) Get() *NetworkingV1AwsPrivateLinkAttachmentStatus {
	return v.value
}

func (v *NullableNetworkingV1AwsPrivateLinkAttachmentStatus) Set(val *NetworkingV1AwsPrivateLinkAttachmentStatus) {
	v.value = val
	v.isSet = true
}

func (v NullableNetworkingV1AwsPrivateLinkAttachmentStatus) IsSet() bool {
	return v.isSet
}

func (v *NullableNetworkingV1AwsPrivateLinkAttachmentStatus) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableNetworkingV1AwsPrivateLinkAttachmentStatus(val *NetworkingV1AwsPrivateLinkAttachmentStatus) *NullableNetworkingV1AwsPrivateLinkAttachmentStatus {
	return &NullableNetworkingV1AwsPrivateLinkAttachmentStatus{value: val, isSet: true}
}

func (v NullableNetworkingV1AwsPrivateLinkAttachmentStatus) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableNetworkingV1AwsPrivateLinkAttachmentStatus) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
